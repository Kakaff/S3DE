...
namespace S3DE.Maths
{
    public sealed class Matrix4x4
    {
        ...
        public static Matrix4x4 CreateRotationMatrix(System.Numerics.Quaternion quat)
        {
            ...
            System.Numerics.Vector3 wv = quat.XYZ() * new System.Numerics.Vector3(quat.W);
        ...
        public static Matrix4x4 operator *(Matrix4x4 m1, Matrix4x4 m2)
        {
            ...
            for (int i = 0; i < 4; i++)
                ...
                {
                    m[i, j] =
                        (new System.Numerics.Vector4(m1[i, 0], m1[i, 1], m1[i, 2], m1[i, 3]) *
                        new System.Numerics.Vector4(m2[0, j], m2[1, j], m2[2, j], m2[3, j]))
                        .Sum();
                }
        ...
        {
            /*
            ByteBuffer buff = ByteBuffer.Create(64);
            
            for (int i = 0; i < 4; i++)
                buff.AddRange(4, mV[i]);
            */

            byte[] buff = new byte[64];

            for (int i = 0; i < 4; i++)
                Buffer.BlockCopy(mV[i], 0, buff, i * 16, 16);
